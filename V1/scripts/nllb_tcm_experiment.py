...
def apply_terminology_normalization(prediction: str, entities: list[dict[str, str]]) -> str:
    """Normalize controlled terms with deterministic fuzzy replacement."""
    text = prediction.strip()
    missing: list[str] = []
    for ent in unique_entities(entities):
        term_en = ent["en"]
        text = fuzzy_replace_term(text, term_en)
        if not contains_term(text, term_en):
            missing.append(term_en)
    if not missing:
        return text
    suffix = " Terminology: " + "; ".join(missing) + "."
...
def build_config(args: argparse.Namespace) -> ExperimentConfig:
    return ExperimentConfig(
        ...
        data_status="synthetic_placeholder_for_pipeline_development",
    )
...
def check_env(args: argparse.Namespace) -> None:
    ...
    if counts != {"train": 1050, "dev": 225, "test": 225}:
        print("[check-env] warning: split counts differ from the planned 1050/225/225 split")

    terms = load_terms(root / "generated_materials" / "terminology" / "terminology.csv")
    label_counts = Counter(term.entity_type for term in terms)
    print(f"[check-env] terms={len(terms)} entity_types={dict(label_counts)}")
...
def prepare_data(args: argparse.Namespace) -> None:
    ...
    for split, rows in rows_by_split.items():
        ...
        for row in rows:
            injected_source, detected_entities = inject_entities(row["zh"], terms)
            entities = detected_entities or parse_entities(row.get("entities"))
            base = {
                "id": row["id"],
                "split": split,
                "source": row["zh"],
...
def write_translation_file(
    ...
    for row, pred in zip(rows, predictions):
        entities = unique_entities(parse_entities(row.get("entities")))
        pred_clean = clean_prediction(pred)
        pred_norm = apply_terminology_normalization(pred_clean, entities)
...
def generate_lora(args: argparse.Namespace) -> None:
    ...
    jobs = [
        ...
        ("Ours_entity_aware_domain_adapted", "entity", run_dir / "models" / "entity_domain_lora", True),
    ]
    variant = getattr(args, "variant", "both")
    if variant != "both":
        jobs = [job for job in jobs if job[0] == variant]
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    ablation_map = [
        ...
        ("w/o terminology normalization", "Terminology normalization post-processing", "Ours_entity_aware_domain_adapted"),
    ]
    for setting, removed, model_id in ablation_map:
        base = dict(summary_by_id.get(model_id, {}))
        if not base:
            continue
        if setting == "w/o terminology normalization" and model_id in all_rows:
            rows = all_rows[model_id]
            refs = [row["reference"] for row in rows]
            raw_field = "prediction_clean" if rows and "prediction_clean" in rows[0] else "prediction_raw"
            raw_preds = [row[raw_field] for row in rows]
            scores = score_sacrebleu(raw_preds, refs)
            raw_ta, raw_tcr, _ = score_terms(rows, raw_field)
            base.update(
                {
                    "BLEU": round(scores["BLEU"], 4),
...
def all_pipeline(args: argparse.Namespace) -> None:
    ...
    if args.no_auto_tune:
        print("[all] success criteria not met; auto-tune disabled")
        return
...
def parse_args() -> argparse.Namespace:
    ...
    for command in ["check-env", "download-model", "prepare", "translate-baseline", "evaluate", "all"]:
        ...
        add_common_args(sub)
        if command == "all":
            sub.add_argument("--no-auto-tune", action="store_true")
